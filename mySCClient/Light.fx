...
LIGHTEDCOLOR DirectionalLight(int i, float3 vNormal, float3 vToCamera, float4 vSpecular)
{
	LIGHTEDCOLOR output = (LIGHTEDCOLOR)0;

	//float fTestSpecFactor = 150.0f;
	...
	if (fDiffuseFactor > 0.0f)
	{
		...
		//if(vSpecular.a != 0.0f)
		{
			...
			float3 vReflect = reflect(-vToLight, vNormal);
			//float fSpecularFactor = pow(max(dot(vReflect, vToCamera), 0.0f), gMaterial.m_cSpecular.a);
			//float fSpecularFactor = pow(max(dot(vReflect, vToCamera), 0.0f), vSpecular.a);
			float fSpecularFactor = pow(max(dot(vReflect, vToCamera), 0.0f), fTestSpecFactor);
#else
#ifdef _WITH_LOCAL_VIEWER_HIGHLIGHTING
			float3 vHalf = normalize(vToCamera + vToLight);
#else
...
#endif
			//float fSpecularFactor = pow(max(dot(vHalf, vNormal), 0.0f), gMaterial.m_cSpecular.a);
			float fSpecularFactor = pow(max(dot(vHalf, vNormal), 0.0f), vSpecular.a);
			//float fSpecularFactor = pow(max(dot(vHalf, vNormal), 0.0f), fTestSpecFactor);
#endif
			//output.m_cSpecular = gMaterial.m_cSpecular * (gLights[i].m_cSpecular * fSpecularFactor);
			output.m_cSpecular = vSpecular * (gLights[i].m_cSpecular * fSpecularFactor);
			//output.m_cSpecular = float4(0.5f, 0.5f, 0.5f, 1);//vSpecular * (gLights[i].m_cSpecular * fSpecularFactor);
		}
		output.m_cDiffuse = gMaterial.m_cDiffuse * (gLights[i].m_cDiffuse * fDiffuseFactor);
	}
...
LIGHTEDCOLOR PointLight(int i, float3 vPosition, float3 vNormal, float3 vToCamera, float4 vSpecular)
{
	LIGHTEDCOLOR output = (LIGHTEDCOLOR)0;
...
LIGHTEDCOLOR SpotLight(int i, float3 vPosition, float3 vNormal, float3 vToCamera, float4 vSpecular)
{
	LIGHTEDCOLOR output = (LIGHTEDCOLOR)0;
	float3 vToLight = gLights[i].m_vPosition - vPosition;
	float fDistance = length(vToLight);
...
float4 Lighting(float3 vPosition, float3 vNormal, float4 vSpecular)
{
	int i;
	float3 vCameraPosition = float3(gvCameraPosition.x, gvCameraPosition.y, gvCameraPosition.z);
	float3 vToCamera = normalize(vCameraPosition - vPosition);
	...
	for (i = 0; i < MAX_LIGHTS; i++)
	{
		if (gLights[i].m_bEnable == 1.0f)
		{
			if (gLights[i].m_nType == DIRECTIONAL_LIGHT)
			{
				LightedColor = DirectionalLight(i, vNormal, vToCamera, vSpecular);
				cColor += (LightedColor.m_cAmbient + LightedColor.m_cDiffuse + LightedColor.m_cSpecular);
			}
			if (gLights[i].m_nType == POINT_LIGHT)
			{
				LightedColor = PointLight(i, vPosition, vNormal, vToCamera, vSpecular);
				cColor += (LightedColor.m_cAmbient + LightedColor.m_cDiffuse + LightedColor.m_cSpecular);
			}
			if (gLights[i].m_nType == SPOT_LIGHT)
			{
				LightedColor = SpotLight(i, vPosition, vNormal, vToCamera, vSpecular);
				cColor += (LightedColor.m_cAmbient + LightedColor.m_cDiffuse + LightedColor.m_cSpecular);
			}
		}
	}
